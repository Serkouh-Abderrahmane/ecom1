        ...
        <aside>
            <div class="m-sidebar">
                <div class="m-sidebar__header">
                    ...
                    <?php if (isset($_GET['category']) && $_GET['category'] != '') { ?>
                        <a href="javascript:void(0);" class="clear-filter" data-type-clear="category" style="font-size:13px;color:rgb(var(--color-foreground-secondary));">&#10005;</a>
                    <?php } ?>
                </div>
                <a href="javascript:void(0);" id="show-xs-nav" style="display:flex;justify-content:space-between;padding:10px;border:1px solid rgb(var(--color-border));font-size:14px;margin-bottom:12px;">
                    ...
                    function loop_tree_shop($pages, $is_recursion = false) { ?>
                        <ul style="<?= $is_recursion ? 'margin-left:16px;' : '' ?>">
                            <?php foreach ($pages as $page) {
                                ...
                                <li>
                                    <a href="javascript:void(0);" data-categorie-id="<?= $page['id'] ?>" class="m-sidebar__item go-category">
                ...
                <div class="m-sidebar" style="margin-top:20px;">
                    <div class="m-sidebar__header">
                        ...
                        <?php if (isset($_GET['brand_id']) && $_GET['brand_id'] != '') { ?>
                            <a href="javascript:void(0);" class="clear-filter" data-type-clear="brand_id" style="font-size:13px;color:rgb(var(--color-foreground-secondary));">&#10005;</a>
                        <?php } ?>
                    </div>
                    <?php foreach ($brands as $brand) { ?>
                        <a href="javascript:void(0);" data-brand-id="<?= $brand['id'] ?>" class="m-sidebar__item brand"><?= $brand['name'] ?></a>
                    <?php } ?>
                </div>
            <?php } ?>
            <?php if ($showOutOfStock == 1) { ?>
                <div class="m-sidebar" style="margin-top:20px;">
                    ...
                    <a href="javascript:void(0);" data-in-stock="1" class="m-sidebar__item in-stock"><?= lang('in_stock') ?> (<?= $countQuantities['in_stock'] ?>)</a>
                    <a href="javascript:void(0);" data-in-stock="0" class="m-sidebar__item in-stock"><?= lang('out_of_stock') ?> (<?= $countQuantities['out_of_stock'] ?>)</a>
                </div>
            <?php } ?>
